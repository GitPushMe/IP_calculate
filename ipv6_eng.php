		...
		<?php 
				...
				if (!empty($_SESSION['login'])){
 				...
 				if(isset($_POST['exit'])){
 					...
 					session_destroy();
 					header('Location:'. BEZ_HOST .'index.php');
...
if (empty($_REQUEST['network'])){
	tr('Use IP & CIDR Netmask:&nbsp; 	&nbsp;', '2002:4559:1fe2::4559:1fe2/56');
	print $end ;
	exit ;
}
...
// Print Results
echo '<br> Inserted data: <b>' ,"$superNet",'</b>';
echo '<br>';
echo '<br> Data: </b>';
echo '<br> Subnet mask: <b>' ,''.inet_ntop($charMask)."", '</b>';
echo '<br> Network address: <b>',''.inet_ntop($charNet)."", '</b>';
echo '<br> Gateway address by default (default gateway): <b>' ,''.inet_ntop($charNet)."",'</b>';
echo '<br> Minimum IP on the client: <b>' ,''.inet_ntop($charHostMin).'','</b>';
echo '<br> Мaximum IP on the client: <b>' ,''.inet_ntop($charHostMax).'','</b>';
echo '<br> A broadcast address (broadcast): <b>' ,''.inet_ntop($charBcst).'','</b>';
echo '<br> Count of public network addresses: <b>' ,"$host_total_pub",'</b>';
echo '<br> Count of public addresses on the client:  <b>' ,"$host_total",'</b>';
